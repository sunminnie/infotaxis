...
class RectangularPlume(object):
    ...
    def sample(self, pos, t):
        ...
        offset = np.array([0.2, 0]) if np.abs(pos[1]-self.src_pos[1])<0.1 else np.array([0, 0.5])
        hit_rate = get_hit_rate(
            xs_src=xs_src, ys_src=ys_src, pos=offset+self.src_pos,
            w=self.w, d=self.d, r=self.r, a=self.a, tau=self.tau)[0, 0]
